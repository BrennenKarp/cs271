'''
This assember turns the HDL symbol code into binary codes that our program can understand to run our other programs. This uses a symbol table to show what symbols correspond with what binary digits, and a parser to break the code up into smaller elements for easier translation.
'''
...
class Assembler(object):
    ...
    def assemble(self, asm_filename):
        ...
        parser = self.parser
        while parser.has_more_instruction:
            parser.advance()
...
class Code(object):
    ...
    def comp(self, sym):
        ...
        if 'M' in sym:
            a_bit = '1'
            sym = sym.replace('M', 'A')
        c_bit = comp_dict.get(sym, '000000')
        return a_bit + c_bit
